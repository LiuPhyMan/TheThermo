...
def H(*, i, j, p, q, CI):
    coeffs
    A1 = sum([_c[0] * mu1**(_c[1][0]) * mu2**(_c[1][1]) * CI.getcolliIntgrl(index=_c[2]) for _c
              in coeffs(p, q)])
    A2 = sum([_c[0] * mu1**(_c[1][0]) * mu2**(_c[1][1]) * CI.getcolliIntgrl(index=_c[2]) for _c
              in coeffs(p, q)])
    ...
    for iRow in range(K):
        for jCol in range(K):
            ...
            for _c in coeffs1(p, q):
                tmp = tmp + _c[0] * M1**_c[1][0] * M2**_c[1][1] * CI.getColliIntegral(index=(_c[2]))
...
# two order
b20 = np.linalg.solve(H00 - np.dot(H01.dot(np.linalg.inv(H11)), H10), 2/kB/T*xj)

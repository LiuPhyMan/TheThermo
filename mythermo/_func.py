...
data = np.loadtxt(str(parentPath/'data'/'Gaunt'/'gauntff.dat'),
                  comments='#', encoding='utf')
interp = interp2D(
  (np.arange(-16, 13 + 0.2, 0.2), np.arange(-6, 10 + 0.2, 0.2)), data)
...
gffint_data = np.loadtxt(str(parentPath/'data'/'Gaunt'/'gauntff_freqint.dat'),
                         skiprows=37)
x = gffint_data[:, 0]
y = gffint_data[:, 1]

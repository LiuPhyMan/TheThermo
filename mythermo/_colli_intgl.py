...
from math import pi, log, sqrt, gamma, exp
from pathlib import Path

import myconst as const
import numpy as np
from myconst import eV2K, K2eV, relM2absM, epsilon_0, kB
...
factorPath = parentPath/'data'/'colli-integral'
...
class _AbsColliIngrl(object):
  ...
  def get_avg_CS(self, *, index: tuple):
    return self.getColliIntegral(index=index)/sqrt(
      kB*self.rdcdT/2/pi/self.rdcdM)/ \
...
class _AbsImpovedLJColli(_AbsColliIngrl):
  ...
  def _set_paras(self):
    ...
    for _index in (
        "1-1", "1-2", "1-3", "1-4", "1-5", "2-2", "2-3", "2-4", "3-3", "4-4"):
      self.c_mtx_dict[_index] = np.zeros((7, 3))
      self.c_mtx_dict[_index][:4, :2] = np.loadtxt(
        str(self._path/f"{_index}.txt"),
        delimiter=",", skiprows=0, max_rows=4)
      self.c_mtx_dict[_index][4:, :3] = np.loadtxt(
        str(self._path/f"{_index}.txt"),
...
class EmptyColli(_AbsColliIngrl):
...
class LJColli(_AbsColliIngrl):
  ...
  def _setParas(self):
    ...
    data = np.loadtxt(str(factorPath/'LJ(12-6).txt'), delimiter=",")
    # 11, 12, 13, 22, 23, 24
...
class LJm6ColliIntegral(_AbsImpovedLJColli):
  ...
  def __init__(self, *, pairs_str: str, style: str, coeff_str: str):
    ...
    """
    super().__init__(pairs_str=pairs_str, style=style, coeff_str=coeff_str)
    self._path = factorPath/"LJm6"
    self._coef = [0.8002, 0.049256]
    super()._set_paras()


class LJm4ColliIntegral(_AbsImpovedLJColli):

  def __init__(self, *, pairs_str: str, style: str, coeff_str: str):
    super().__init__(pairs_str=pairs_str, style=style, coeff_str=coeff_str)
    self._path = factorPath/"LJm4"
...
class ExpColli(_AbsColliIngrl):
  ...
  def _setParas(self):
    data = np.loadtxt(str(factorPath/'Exp.txt'), delimiter=",")
    self.interp_f11 = interp1d(data[:, 0], data[:, 1])
    self.interp_f12 = interp1d(data[:, 0], data[:, 2])
...
#             log_T = np.log10(
#                 [0.004, 0.01, 0.02, 0.04, 0.1, 0.2, 0.4, 1, 2, 4, 10, 20,
#                 40, 100, 200])
#             _pos_11 = (3, 15)
#             _pos_22 = (43, 15)
#         elif 2 <= self.Paras["C"] <= 4:
#             log_C = np.log10([2, 4])
#             log_T = np.log10([0.004, 0.01, 0.02, 0.04, 0.1, 0.2, 0.4, 1,
#             2, 4])
#             _pos_11 = (20, 10)
#             _pos_22 = (60, 10)
...
#         if index == (1, 1):
#             return self.f_11(
#                 np.log10(self.rdcdT / De_T)) * self.tempValue * sqrt(
#                 self.rdcdT) * 1
#         if index == (2, 2):
#             return self.f_22(
#                 np.log10(self.rdcdT / De_T)) * self.tempValue * sqrt(
...
# =============================================================================================== #
class ScreenCoulombColli(_AbsColliIngrl):
  ...
  def __init__(self, *, pairs_str: str, style: str, coeff_str: str):
    ...
    Collision integrals for charged-charged interaction in two-temperature
...
# ----------------------------------------------------------------------------------------------- #
class EleNeuColliIntegral(_AbsColliIngrl):
  ...
  def getColliIntegral(self, *, index: tuple):
    ...
    def f2(y):
      ...
      if abs(y) < 1e-15:
        return 0
      x2 = (s + 1)/abs(y)
      return (s + 1)/abs(y)**2*exp(-x2)*x2**(s + 1)*self.Q_f(
        x2*self.rdcdT*K2eV)
...
# ----------------------------------------------------------------------------------------------- #
class TableColliIntegral(_AbsColliIngrl):
